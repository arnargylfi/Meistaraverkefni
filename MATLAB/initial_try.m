...
filtered_hus = hus(ismember(hus.L_sing, flokkar_sem_vil),:);

heildar_fermetrar = sum(filtered_hus.fermetrar);

% Mjög rough estimate hérna. 
medal_lofthaed = 2.5;
heildar_rummal = medal_lofthaed*heildar_fermetrar;
heildar_dagleg_orkuthorf = heildar_rummal*energy_need/365 %kWh
orkuthorf_per_hus = heildar_dagleg_orkuthorf/height(filtered_hus) %kWh
...
%% Plotta potential daily power miðað við estimated wind í gefinni hæð og daglega orkuþörf
Cp = 0.4;                  % Typical coefficient of performance
air_density = 1.225;       % kg/m^3, við sjávarmál, þarf kannski að reikna með minnkun í aukinni hæð
blade_length = 20;         % m
time_step_hours = 10 / 60; % 10-minute interval in hours

% Compute power output at each timestep
Power = potential_W(Cp, air_density, blade_length, V_estimate);

% Convert to energy in kWh per timestep
Energy_per_timestep = Power * time_step_hours / 1000;

% Group energy by day
daily_energy = accumarray(day(timi, 'dayofyear'), Energy_per_timestep, [], @sum);

% Plot
figure;
plot(1:length(daily_energy), daily_energy, 'b-', 'LineWidth', 1.5);
hold on 
yline(heildar_dagleg_orkuthorf,'r','LineWidth',1.5)
hold on 
plot(1:length(daily_estimate),daily_estimate)
xlabel('Dagur árs');
ylabel('Dagleg orka (kWh)');
title('Vindorkuframleiðsla yfir dag');
grid on;
